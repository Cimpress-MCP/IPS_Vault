...
aws s3api list-objects --bucket ${BUCKET} --prefix public-keys/ --region us-east-1 --output text --query 'Contents[?Size>`0`].Key' | sed -e 'y/\t/\n/' > ~/keys_retrieved_from_s3
while read line; do
  ...
  if [[ "$USER_NAME" =~ ^[a-z][-a-z0-9]*$ ]]; then
    ...
    if [ -f ~/keys_installed ]; then
      ...
      if [ $? -eq 0 ]; then
        aws s3 cp s3://${BUCKET}/$line /home/$USER_NAME/.ssh/authorized_keys --region us-east-1
